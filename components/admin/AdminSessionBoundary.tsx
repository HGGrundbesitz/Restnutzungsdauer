...
export default function AdminSessionBoundary({
  ...
  useEffect(() => {
    ...
    const verifyAdminSession = async (candidate: Session | null) => {
      ...
      if (membershipError) {
        ...
        setSession(null);
        setSessionError(
          'Die Admin-Berechtigung konnte nicht geprüft werden. Bitte führen Sie zuerst das Supabase-SQL-Setup aus.',
        );
        setLoading(false);
        return;
      }
    ...
    const loadSession = async () => {
      if (!isSupabaseConfigured) {
        setSessionError(
          'Supabase ist nicht vollständig konfiguriert. Prüfen Sie URL und Publishable Key in der .env.local.',
        );
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (isMounted) {
          setSession(null);
          setSessionError(
            'Supabase konnte nicht erreicht werden. Prüfen Sie SUPABASE_URL und starten Sie den Dev-Server neu.',
          );
          setLoading(false);
        }
      }
    ...
    const {
      ...
      window.setTimeout(() => {
        if (isMounted) void verifyAdminSession(nextSession);
      }, 0);
    });
  ...
  if (!session) return <AdminLogin externalError={sessionError} />;
