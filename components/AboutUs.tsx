    ...
    <section id="warum-wir" className="relative overflow-hidden border-t border-[var(--color-border)] bg-[var(--color-bg)] py-24 md:py-32">
      <div className="section-shell relative z-10">
        <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2 lg:gap-16">
          <motion.div initial={{opacity: 0, x: -30}} whileInView={{opacity: 1, x: 0}} viewport={{once: true, margin: '-100px'}} transition={{duration: 0.8}} className="flex flex-col">
            <div className="section-eyebrow mb-6 w-fit"><Award size={12} className="text-[var(--color-accent)]" />Vertrauen</div>
            <h2 className="font-heading text-4xl leading-[1.02] tracking-tight text-[var(--color-ink)] md:text-5xl">Warum wir die richtigen sind:</h2>
            <div className="mt-8 space-y-4">
              {reasons.map((reason) => (
                <div key={reason} className="theme-panel flex items-start gap-4 rounded-[1.4rem] p-5 shadow-[0_18px_44px_-34px_rgba(15,23,42,0.28)]">
                  ...
                  <p className="text-base font-semibold leading-7 text-[var(--color-text-muted)]">{reason}</p>
                </div>
              ))}
            </div>
          ...
          <motion.div initial={{opacity: 0, x: 30}} whileInView={{opacity: 1, x: 0}} viewport={{once: true, margin: '-100px'}} transition={{duration: 0.8, delay: 0.2}} className="group relative min-h-[460px] overflow-hidden rounded-[2.2rem] border border-[var(--color-border)] bg-[var(--color-bg-alt)] shadow-2xl md:min-h-[560px]">
