...
import AfaComparison from '@/components/AfaComparison';
...
export default function HomePage() {
  ...
  return (
    <main className="overflow-x-hidden text-[var(--color-ink)]">
      <motion.div
        className="fixed left-0 right-0 top-0 z-[100] h-1 origin-left bg-[var(--color-accent)]"
        style={{scaleX}}
      />
      <Header />
      <Hero />
      <div ref={quickCheckRef} className="relative z-10 w-full bg-white pt-24 sm:pt-32 lg:pt-40">
        <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 h-56 bg-[radial-gradient(ellipse_at_top,rgba(37,99,235,0.075),transparent_66%)]" />
        <motion.div
          style={reduceMotion ? undefined : {y: quickCheckY, scale: quickCheckScale, opacity: quickCheckOpacity}}
        >
          <QuickCheck />
        </motion.div>
        <RequiredDocuments />
