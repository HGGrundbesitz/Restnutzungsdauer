...
import type {RndInputV1, RndResult, RndWarning} from './types.ts';
import {validateRndInputV1} from './validate-input.ts';

export const RND_MODEL_VERSION_V1 = 'immowertv-2022-immowerta-v1';
export const RND_RESULT_COPY_VERSION_V1 = '2026-07-v1';

const LEGACY_AUTOMATIC_BUILDING_TYPES = new Set([
  'single_family',
  'multi_family',
  'mixed_use_residential',
]);

export function calculateRndV1(input: RndInputV1): RndResult {
  const validation = validateRndInputV1(input);
  ...
  const legacyAutomaticModel = LEGACY_AUTOMATIC_BUILDING_TYPES.has(input.buildingTypeCode);
  ...
  const requiresManualReview =
    gndYears === null
    || !legacyAutomaticModel
    || input.coreRenovation
    || hasUnknownModernizationAnswer(input.modernization);

  if (gndYears === null) {
    return {
      modelVersion: RND_MODEL_VERSION_V1,
      resultCopyVersion: RND_RESULT_COPY_VERSION_V1,
  ...
  const formulaResult = usePreliminaryFormula
    ...
    : coefficient.a * ((ageForFormula ** 2) / gndYears)
      - coefficient.b * ageForFormula
